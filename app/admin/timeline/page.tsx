...
// Palet warna untuk light mode (teks lebih gelap, background transparan lebih ringan)
...
// Status badge untuk light mode (teks gelap, background lebih ringan)
...
const inputStyle: React.CSSProperties = {
  ...
  borderRadius: 8, padding: "8px 12px", color: "#111827",
...
export default function TimelinePage() {
  ...
  const selStyle: React.CSSProperties = { ...inputStyle, width: "auto", padding: "5px 10px", cursor: "pointer" };
  ...
  return (
    <div style={{ padding: "24px 28px", color: "#111827", minHeight: "100vh", background: "#f9fafb", fontFamily: "inherit" }}>

      {/* Header */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
        <div>
          <p style={{ fontSize: 11, letterSpacing: "0.1em", textTransform: "uppercase", color: "#6b7280", margin: "0 0 4px" }}>CV Mitra Koperasi Nusantara</p>
          <h1 style={{ fontSize: 22, fontWeight: 600, margin: 0, color: "#1f2937" }}>Timeline Kegiatan 12 Bulan</h1>
        </div>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          {saving && <span style={{ fontSize: 11, color: "#6b7280" }}>Menyimpan…</span>}
          <button onClick={fetchAll} title="Refresh" style={{ background: "#ffffff", border: "1px solid #d1d5db", borderRadius: 8, padding: "7px 10px", cursor: "pointer", color: "#374151", display: "flex", alignItems: "center" }}>
          ...
          <button onClick={openCreate} style={{ display: "flex", alignItems: "center", gap: 6, background: "#f59e0b", border: "none", borderRadius: 8, padding: "8px 16px", color: "#1c1917", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(120px,1fr))", gap: 10, marginBottom: 24 }}>
        {[
          ...
          <div key={s.label} style={{ ...card, padding: "12px 16px" }}>
            ...
            <p style={{ fontSize: 26, fontWeight: 700, margin: 0, color: s.color }}>{s.val}</p>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", marginBottom: 16 }}>
        <div style={{ display: "flex", border: "1px solid #d1d5db", borderRadius: 8, overflow: "hidden" }}>
          {(["timeline","tabel"] as const).map(v => (
            <button key={v} onClick={() => setView(v)} style={{ padding: "6px 14px", border: "none", cursor: "pointer", fontSize: 12, fontWeight: 500, fontFamily: "inherit", background: view === v ? "#f59e0b" : "transparent", color: view === v ? "#1c1917" : "#374151", textTransform: "capitalize" }}>
              {v}
            </button>
          ))}
        </div>
        <select value={filterKat === "Semua" ? "Semua" : filterKat} onChange={e => setFilterKat(e.target.value === "Semua" ? "Semua" : Number(e.target.value))} style={selStyle}>
          <option value="Semua">Semua Kategori</option>
          {katList.map(k => <option key={k.id} value={k.id}>{k.nama}</option>)}
        </select>
        <select value={filterStt} onChange={e => setFilterStt(e.target.value)} style={selStyle}>
          <option>Semua</option>
          {STATUS_LIST.map(s => <option key={s}>{s}</option>)}
        </select>
        <span style={{ fontSize: 11, color: "#6b7280", marginLeft: "auto" }}>Klik status = ganti • Klik baris = edit</span>
      </div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "6px 20px", marginBottom: 16 }}>
        {katList.map(k => {
          const w = getW(k.id);
          return (
            <span key={k.id} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: "#374151" }}>
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, tableLayout: "fixed" }}>
                    ...
                    <tbody>
                      {items.map((item, idx) => {
                        ...
                        const stt = getS(item.status);
                        return (
                          <tr key={item.id} onClick={() => openEdit(item)} style={{ borderBottom: idx < items.length-1 ? "1px solid #e5e7eb" : "none", cursor: "pointer", transition: "background 0.1s" }}
                            ...
                            <td style={{ padding: "8px 10px" }}>
                              <span onClick={e => { e.stopPropagation(); cycleStatus(item); }} title="Klik ganti status"
                            ...
                            <td style={{ padding: "8px 10px", textAlign: "center" }}>
                              <button onClick={e => { e.stopPropagation(); del(item.id); }} style={{ background: "none", border: "none", cursor: "pointer", color: "#6b7280", padding: 2, display: "flex", alignItems: "center" }} title="Hapus">
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16, zIndex: 999 }}>
          <div style={{ background: "#ffffff", border: "1px solid #e5e7eb", borderRadius: 16, width: "100%", maxWidth: 480, maxHeight: "90vh", overflowY: "auto" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 20px", borderBottom: "1px solid #e5e7eb" }}>
              ...
              <button onClick={() => setModal(false)} style={{ background: "none", border: "none", cursor: "pointer", color: "#6b7280", fontSize: 18, lineHeight: 1 }}>✕</button>
            </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 20px", borderTop: "1px solid #e5e7eb" }}>
              {editId
                ? <button onClick={() => del(editId)} style={{ background: "none", border: "none", cursor: "pointer", color: "#dc2626", fontSize: 13, display: "flex", alignItems: "center", gap: 5, fontFamily: "inherit" }}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clipRule="evenodd"/></svg>
                    Hapus
                  </button>
                : <div />
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => setModal(false)} style={{ padding: "7px 16px", borderRadius: 8, border: "1px solid #d1d5db", background: "transparent", color: "#374151", fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>Batal</button>
                <button onClick={submit} disabled={saving || !form.kegiatan.trim()} style={{ padding: "7px 18px", borderRadius: 8, border: "none", background: form.kegiatan.trim() ? "#f59e0b" : "#e5e7eb", color: form.kegiatan.trim() ? "#1c1917" : "#9ca3af", fontSize: 13, fontWeight: 600, cursor: form.kegiatan.trim() ? "pointer" : "not-allowed", fontFamily: "inherit" }}>
